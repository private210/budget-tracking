...
- **Export PDF preview popup (2026-08-08):** tombol PDF di `reports/index` tidak langsung download — buka modal `#pdfPreviewModal` (pop-up, backdrop blur, close via tombol X/Escape/klik luar) yang menampilkan **pratinjau sheet mirip layout PDF** (header wordmark, kartu Gaji/Pengeluaran/Sisa, bar per kategori, bar harian 31 hari, tabel rincian lengkap + TOTAL) lalu tombol **Download PDF** → `reports.export` + `showLoading()`. Data preview pakai variabel yang sudah ada di view + `$expenses` (query baru di `ReportController::index`) + `$previewDays` (dibangun di blade dari `$dailyExpenses`, pola `substr((string)$d->date, 8, 2)`). JS: `openPdfPreview(e)` / `closePdfPreview()` di `@push('scripts')`. Tombol Excel tetap langsung download. **Aturan loading bar tetap: link yang menavigasi harus panggil `showLoading()`** (Download PDF di modal sudah, XLSX sudah, tombol PDF sendiri `preventDefault` tanpa navigasi jadi tidak perlu).
