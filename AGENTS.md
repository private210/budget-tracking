...
- **Export PDF preview halaman penuh (2026-08-08, gaya Google Drive):** tombol PDF di `reports/index` adalah link navigasi → `GET /reports/preview/{format}` (`ReportController::previewPage`, view `reports/preview.blade.php`) — toolbar (nama file, Kembali, **Download**) + iframe PDF setinggi viewport. File stream = route `GET /reports/preview/{format}/file` (`reports.previewFile` → `ReportController::preview` → Content-Disposition `inline`). `SecurityHeaders` meng-exempt X-Frame-Options hanya untuk `reports.previewFile`. Tombol PDF/Download/Kembali pakai `showLoading()` (navigasi nyata). **Pop-up modal dihapus total** (HTML + JS `openPdfPreview`/`closePdfPreview`) — menyelesaikan bug scope global inline `onclick` selamanya. Aturan: JANGAN pakai pop-up/JS-inline untuk preview — **inline `onclick` butuh fungsi di scope global (window)**, jangan definisikan di dalam callback listener apapun.
