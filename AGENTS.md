...
- **Vercel + dompdf TANPA gd (2026-08-08, fix akar masalah):** **vercel-php@0.7.4 TIDAK punya ekstensi `gd`** (baru ada di 0.9.0/PHP 8.5; list: apcu bcmatch bz2 calendar ctype curl dom dütü... tanpa gd/imagick). Ekstensi <img> PNG di PDF → `Call to undefined function` implisit → 500 **hanya di prod** (lokal punya gd). Fix: header PDF tidak lagi pakai `<img logo-light.png>` — diganti teks brand (`<span class="brand-mark">T</span>` + "Titik Simpan", CSS murni). **Aturan: jangan taruh <img> apapun di view PDF** kalau gambar tidak base64 & backend tidak ada. Upgrade runtime ke vercel-php@0.9.x kalau ingin gambar kembali (butuh test PHP 8.5).
