...
- **Kembali ke teks logo + PHP 8.3 (2026-08-08, fix `dd1c52d`+):** 0.9.0 (PHP 8.5) memunculkan deprecation `PDO::MYSQL_ATTR_SSL_CA deprecated` (dari `config/database.php` options mysql, dievaluasi saat config dimuat) — runtime dikembalikan ke **vercel-php@0.7.4** (PHP 8.3), header PDF kembali **teks brand** (tanpa kotak "T", cuma wordmark "Titik Simpan" — kotak hijau dihapus 2026-08-08). **Ukuran PDF kecil**: `config/dompdf.php` `enable_font_subsetting => true` → PDF ~74KB (sebelumnya 927KB full DejaVu embed). Margin `@page` 16px + `body padding` 8px (compact, tidak penuh halaman). Footer satu baris tetap.
