...
- **Vercel + dompdf 500** (2026-08-08, fix `808aed9`): storage read-only di Vercel → dompdf gagal menulis font cache ke `storage_path('fonts')` → 500 di download PDF. Fix: `config/dompdf.php` (copy penuh dari vendor, 3 key pakai env: `font_dir`/`font_cache`/`temp_dir`) + env di `vercel.json` (`DOMPDF_FONT_DIR=/tmp/dompdf_fonts`, dll) + guard `@mkdir` di `ReportController::exportPdf`. **Jangan** pakai config dompdf minimal sendiri — chroot hilang → logo PNG di PDF ikut hilang (PDF 73KB vs normal 1MB). Cache font (`storage/fonts/*.json`) jangan di-commit (sudah di .gitignore).
