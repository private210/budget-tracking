...
- **Export PDF preview popup (2026-08-08, revisi iframe):** tombol PDF di `reports/index` buka modal `#pdfPreviewModal` yang menampilkan **PDF asli via iframe** — `src="{{ route('reports.preview', ...) }}"` (route baru `GET /reports/preview/{format}`, `where format = pdf`, `ReportController::preview()` → `renderPdf($data)->stream()` = Content-Disposition `inline`). Tombol **Download PDF** di modal → `reports.export` + `showLoading()`. `SecurityHeaders` meng-exempt `X-Frame-Options` untuk route `reports.preview` (jika tidak, iframe diblokir DENY). Data dibangun `reportData($month)` (shared oleh `export`, `preview`); view `reports.pdf` menerima array itu langsung (`Pdf::loadView('reports.pdf', $data)`). Tombol PDF sendiri `preventDefault` (tanpa navigasi, tidak perlu `showLoading`); XLSX tetap langsung download. **BUG (2026-08-08): `openPdfPreview`/`closePdfPreview` awalnya didefinisikan di dalam callback `DOMContentLoaded` (halaman Chart.js) → bukan fungsi global → `onclick` inline `ReferenceError` → fallback `href="#"` (URL jadi `/reports#`, modal tak muncul).** Aturan: **inline `onclick` butuh fungsi di scope global (window)** — jangan definisikan di dalam callback listener apapun. **Sebelumnya (static HTML sheet) dibuang: file lebih kecil ~140 baris** — ini hasil dedup review ponytail.
