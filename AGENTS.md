...
## Session Memory (last updated: 2026-08-09)

### Deployment status

- **Production = Vercel + Neon PostgreSQL** (`https://titik-simpan.vercel.app`), auto-deploy from GitHub push to `master`
- **Isolasi data per pengguna LIVE** since commit `1063b12` (2026-08-09): semua tabel business punya global scope `user_id` (`ScopedByUser` trait di 5 model), akun baru otomatis dapat 6 kategori default, profil bisa upload foto (base64 → `avatar`, kolom jadi `longText`) + ganti password (StrongPassword rule: 10+ karakter, huruf besar/kecil, angka, simbol)
- **TODO (manual, user must do):** (1) update OAuth credentials at console.cloud.google.com with redirect URI `https://titik-simpan.vercel.app/auth/google/callback`; (2) update `APP_URL` + `GOOGLE_REDIRECT_URI` to `https://titik-simpan.vercel.app` on Vercel env; (3) run google migration on Neon; (4) **run multi-user migration SQL on Neon** (migration tidak auto-run di Vercel): `ALTER TABLE categories/salaries/budget_allocations/expenses/recurring_expenses ADD COLUMN user_id bigint NULL;` + UPDATE backfill ke user pertama + index + `ALTER TABLE users ALTER COLUMN avatar TYPE text;` — SQL lengkap di commit message `1063b12` / kode migrasi `2026_08_09_000001`
...
- **Multi-user isolation** (2026-08-09, `1063b12`): trait `app/Models/Concerns/ScopedByUser.php` — global scope `where('user_id', auth id)` + auto-assign di event `creating` (hanya saat `auth()->check()`). Dipakai Category, Salary, BudgetAllocation, Expense, RecurringExpense. Implikasi: (1) query apa pun di model itu otomatis ter-filter per user, (2) `CategorySeeder::seedFor(User)` dipanggil `AuthController` saat register/Google akun baru (6 kategori default), (3) `AugDemoSeeder` baru jalan: `Auth::loginUsingId(user pertama)` di awal `run()` — tanpa itu, data dibuat tanpa `user_id` = tidak terlihat siapa pun. Seeder lain/user_id null-pun tidak terlihat. Migrasi `2026_08_09_000001` guarded (`hasColumn`), backfill data lama → user pertama.
- **Profil lengkap** (`1063b12`): `ProfileController::update` — name/email + upload foto `avatar` (base64 data URI masuk kolom `avatar` jadi `longText` via migrasi `2026_08_09_000002`, mimes jpg/jpeg/png/webp max 1MB) + ganti password: user Google tanpa kata sandi lama, user biasa wajib `current_password` benar. Hint di view: file input JPG/PNG/WEBP.
- **StrongPassword rule** (`app/Rules/StrongPassword.php`): min 10, wajib a-z, A-Z, 0-9, simbol; dipakai register + profile update; pesan Indonesia.
...
- **UI interaktif pass** (2026-08-09, `b71c6f9`): progress bar alokasi dashboard animasi 0%→target via anime.js (`.alloc-bar`, `data-width`), toggle aktif/nonaktif berulang jadi switch native (checkbox `sr-only` + `onchange="showLoading(); this.form.submit()"` — wajib showLoading sebelum form.submit), label frekuensi Indonesian (weekly/monthly/yearly → Mingguan/Bulanan/Tahunan), preview foto profil live di `/profile` (`previewAvatar()` FileReader, cek max 1MB sebelum baca)
...
- **Export dgn grafik + footer** (2026-08-08): PDF dirombak — `@page margin 16/14/22mm`, brand header, kartu Gaji/Pengeluaran/Sisa, grafik CSS (bar per kategori pakai `$catColors` + bar harian 31 sel `<table class="daily">`), footer fixed `position: fixed` dgn `counter(page)` (class `.page-number::after`). Excel dapat sheet tambahan **Ringkasan** (Gaji/Pengeluaran/Sisa di A4:B6, tabel Kategori di A8 dengan series label `Ringkasan!\$A\$8`) + grafik bar asli pakai `PhpSpreadsheet\Chart` — **harus `(new DataSeries(..., $plotLabel, ...))` dgn `$plotLabel` flat array (jangan dibungkus lagi!) dan `$writer->setIncludeCharts(true)`** atau error `refresh() on array`. `DATE(spent_at)` dari `DB::raw` itu **string** `'Y-m-d'` di sqlite (bukan Carbon) — ambil hari via `substr((string)$d->date, 8, 2)`. Footer semua eksport + layout website pakai copyright `Titik Simpan - Ard Production` (`© {{ now()->year }}`).
- **Vercel + dompdf 500** (2026-08-08, fix `808aed9`): storage read-only di Vercel → dompdf gagal menulis font cache ke `storage_path('fonts')` → 500 di download PDF. Fix: `config/dompdf.php` (copy penuh dari vendor, 3 key pakai env: `font_dir`/`font_cache`/`temp_dir`) + env di `vercel.json` (`DOMPDF_FONT_DIR=/tmp/dompdf_fonts`, dll) + guard `@mkdir` di `ReportController::exportPdf`. **Jangan** pakai config dompdf minimal sendiri — chroot hilang → efek samping lain. Cache font (`storage/fonts/*.json`) jangan di-commit (sudah di .gitignore).
- **Vercel + dompdf TANPA gd (2026-08-08, fix akar masalah):** **vercel-php@0.7.4 TIDAK punya ekstensi `gd`** (baru ada di 0.9.0/PHP 8.5; list: apcu bcmatch bz2 calendar ctype curl dom dütü... tanpa gd/imagick). Ekstensi <img> PNG di PDF → `Call to undefined function` implisit → 500 **hanya di prod** (lokal punya gd). Fix awal: header PDF tanpa `<img>` — teks brand CSS murni. **Aturan: jangan taruh <img> apapun di view PDF** kalau backend tidak punya gd.
- **PDF logo webp + runtime 0.9.0 (2026-08-08, `xxx`):** user minta logo `logo-light.webp` dipakai lagi → runtime Vercel di-upgrade ke **`vercel-php@0.9.0`** (PHP 8.5, punya gd) via `vercel.json`. PDF header pakai `<img src="{{ public_path('assets/logo-light.webp') }}">`. Margin PDF: `@page { margin: 20px 20px; }` (hilangkan body margin/padding). Footer satu baris: `© {year} Titik Simpan - Ard Production • Halaman {n}`. Kalau render gambar di PDF error lagi, cek `dompdf/src/Adapter/CPDF.php` `_convert_to_png()` — `imagecreatefrom$type` missing → butuh ext gd (blame `imagecreatefromwebp`).
- **Export PDF preview modal (2026-08-08, gaya Google Drive):** tombol PDF di `reports/index` adalah `<button>` trigger → `openReportPreview(this)` di partial `reports/partials/preview-modal.blade.php` (di-`@include` di dalam `@section('content')` reports.index — jangan di luar section, Blade `@extends` render layout di footer jadi output include akan mampir sebelum `<html>`). Modal baca `data-preview-url`/`data-download-url`/`data-filename` dari trigger. File stream = route `GET /reports/preview/{format}/file` (`reports.previewFile` → `ReportController::preview` → Content-Disposition `inline`); download = `reports.export`. `SecurityHeaders` meng-exempt X-Frame-Options hanya untuk `reports.previewFile`. Halaman penuh lama (`reports.preview`, view `reports/preview.blade.php`, method `previewPage`) **sudah dihapus**. `iframe.src` di-reset `about:blank` saat modal ditutup. Aturan: JANGAN pakai pop-up/JS-inline untuk preview — **inline `onclick` butuh fungsi di scope global (window)**, jangan definisikan di dalam callback listener apapun.
- **PENTING — dompdf mengabaikan `@page` margin (2026-08-08, fix akar masalah):** `html { margin: 0; padding: 0; }` di `<style>` pdf.blade.php membuat dompdf **sama sekali mengabaikan margin `@page`** → konten full-bleed ke tepi kertas (verified via geometry scan: teks sampai x=0 dan y=841.9 padahal `@page margin` sudah diset). Fix: **JANGAN beri style apapun ke selector `html` di view PDF** — terbukti via isolation test (`html{margin:0}` → full-bleed; tanpa rule itu → margin 51pt kiri/kanan jalan). `body { margin:0; padding:0 }` aman. Footer `position:fixed; bottom:-1.6cm` tetap OK (render di area margin bawah).
- **Sebelumnya (PDF 2026-08-08):** margin `@page` **2cm simetris semua sisi** di kertas **A4** (`config/dompdf.php` `default_paper_size=a4`), `body` padding 0, header wordmark "Titik Simpan", footer satu baris `© {year} Titik Simpan - Ard Production • Halaman {n}`, font subsetting aktif → PDF ~73KB.
- **Seeder demo** (2026-08-08): `AugDemoSeeder` (`database/seeders/`) — isi gaji Rp8.500.000 + alokasi 6 kategori + 32 pengeluaran Agustus + 7 tagihan berulang; idempotent (bersihkan data lama dulu, pola resetData). Jalankan lokal: `php artisan db:seed --class=AugDemoSeeder`. **Di Vercel**: pakai route `GET /seed-demo?token=<DEMO_SEED_TOKEN>` (route `demo.seed` di `routes/web.php`) yang memanggil `Artisan::call('db:seed')` — Vercel tidak punya SSH/artisan. Route ditutup token env `DEMO_SEED_TOKEN` (403/hilang jika env kosong) — set di Vercel dashboard sebelum panggil, hapus env setelah selesai. **Catatan multi-user (2026-08-09)**: seeder minta `Auth::loginUsingId(user pertama)`; kalau ga da user, `run()` return dulu.
...
- **Static files in `public/` do NOT auto-serve on Vercel** — the `vercel.json` catch-all `/(.*)` → `/api/index.php` sends every request to Laravel. Any new `public/` asset must get a route in `routes/web.php` (see the `$asset` loop near the top). If an image shows alt-text on production but works locally, it's a missing asset route. **Semua gambar ada di `public/assets/`** — `logo.svg`, `darkmode-logo.svg`, `icon-light.svg`, `icon-dark.svg`, `icon-monokrom.svg`, `logo-light.png`, `logo-dark.png` (routes `asset.*` di `/assets/...`). `favicon.ico` tetap di root `public/` (route `asset.favicon`). Navbar logo **theme-aware**: `#navbar-logo` berganti `icon-light.svg`/`icon-dark.svg` via JS di `applyTheme()`; footer pakai `icon-monokrom`. PDF header pakai `public_path('assets/icon-light.svg')`. `favicon.ico` is a real ICO (256px PNG payload) — regenerate by screenshotting SVG via headless Edge + wrapping PNG in ICO header.
